...
const emit = defineEmits(['onBrandUpdated'])
...
<template>
    <div class="text-center">
        <v-dialog v-model="dialog" width="800" persistent>
            ...
            <v-card>
                <v-toolbar>
                    <v-toolbar-title class="text-h6"> Редактирование бренда {{ brand.name }}</v-toolbar-title>
                    <div class="text-left">
                        <v-btn icon="mdi-close" @click="dialog = false" class="mr-4"></v-btn>
                        <v-spacer />
                    </div>
                </v-toolbar>
                <v-form @submit.prevent="updateBrand" v-model="form">
...
export default {
    ...
    methods: {
        updateBrand() {
            ...
            axios.patch('http://localhost:8000/brands/' + this.brand.id, this.FormData, {
